// gtk calculator ...git test_1_remote
...
static void activate(GtkApplication* app, gpointer user_data)
{
    ...
    widget.window = gtk_application_window_new(app);//메인 애플리케이션 창 생성
    gtk_window_set_position(GTK_WINDOW(widget.window),GTK_WIN_POS_CENTER);
    gtk_window_set_title(GTK_WINDOW(widget.window),"Las Creation Calculator");
    ...
    box = gtk_entry_new(); //텍스트 입력을 받는 표준 위젯
...
int main(int argc, char** argv)
{
    ...
    int status;
    app = gtk_application_new("org.gtk.calculator",G_APPLICATION_FLAGS_NONE);//gtk애플리케이션 개체 생성

    g_signal_connect(app,"activate",G_CALLBACK(activate),NULL); //gtk의 시그널-슬롯 시스템을 사용하여 이벤트 핸들러를 연결
    status = g_application_run(G_APPLICATION(app),argc,argv);//gtk 애플리케이션 메인 루프 시작
    g_object_unref(app);//애플리케이션 객체의 참조 카운트를 감소 , 참조 타운트가 0이 되면 객체가 메모리에서 해제
